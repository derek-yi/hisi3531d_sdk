...
#define CIPHER_CheckHandle(CIHandle)   \
    ...
    { \
        ...
        if (CIPHER_SOFT_CHAN_NUM <= HI_HANDLE_GET_CHNID(CIHandle)) \
        { \
            HI_ERR_CIPHER("chan %d is too large, max: %d\n", HI_HANDLE_GET_CHNID(CIHandle), CIPHER_SOFT_CHAN_NUM); \
            return HI_ERR_CIPHER_INVALID_PARA; \
        } \
        if (CI_PKGx1_CHAN >= HI_HANDLE_GET_CHNID(CIHandle)) \
        { \
            HI_ERR_CIPHER("chan %d is too samll, min: %d\n", HI_HANDLE_GET_CHNID(CIHandle), CI_PKGxN_CHAN_MIN); \
            return HI_ERR_CIPHER_INVALID_PARA; \
        } \
...
HI_S32 DRV_CIPHER_EncryptEx(CIPHER_DATA_S *pstCIData, HI_BOOL bIsDescrypt)
{
    ...
    {
        HI_ERR_CIPHER("Invalid point: pstCIData is null!\n");
        return HI_ERR_CIPHER_INVALID_PARA;
    }
    ...
    if ((0 != pstCIData->ScrPhyAddr % 4) || (0 != pstCIData->DestPhyAddr % 4)
        || (pstCIData->ScrPhyAddr + pstCIData->ByteLength < pstCIData->ScrPhyAddr)
        || (pstCIData->DestPhyAddr + pstCIData->ByteLength < pstCIData->DestPhyAddr))
    {
        HI_ERR_CIPHER("Error: Invalid ScrPhyAddr, DestPhyAddr or ByteLength!\n");
        return HI_ERR_CIPHER_INVALID_PARA;
    }
...
*/
    ...
    memset(&cipherListBuf, 0, sizeof(cipherListBuf));
