...
#define OSDRV_MODULE_VERSION_STRING "HISI_WDT-MDC030001 @Hi3536v100"

#define HISILICON_SCTL_BASE 0x12050000
/* define watchdog IO */
#define HIWDT_BASE      0x12070000
...
void hidog_checkmodeparam(void)
{
    if((default_margin <= 0)||(default_margin >= 80))
    {
        printk("error,default_margin is (0,80),now set default value 60\n");
		default_margin = 60;
    }

	if((nowayout != 0) && (nowayout != 1))
	{
	    printk("error,nowayout is [0,1],now set default value WATCHDOG_NOWAYOUT\n");
		nowayout = WATCHDOG_NOWAYOUT;
	}

	if((nodeamon != 0) && (nodeamon != 1))
	{
	    printk("error,nodeamon is [0,1],now set default value 0\n");
		nodeamon = 0;
	}
