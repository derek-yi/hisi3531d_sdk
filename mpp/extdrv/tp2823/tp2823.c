...
#define SAV_HEADER          BT656_HEADER_8BIT

static int mode = TP2802_1080P25;
static int chips = 4;
...
static void TP282x_output(unsigned char addr)
{
	...
	//tp28xx_byte_write(addr, 0xFA, 0x88);
	//tp28xx_byte_write(addr, 0xF5, 0x0f);
	//tp28xx_byte_write(addr, 0xFA, 0x00);

    if (MUX_2CH == output)
    {
        tp28xx_byte_write(addr, 0xF3, 0xa8);
        tp28xx_byte_write(addr, 0xF5, 0x0f);
        tp28xx_byte_write(addr, 0xFA, 0x00);
        //2CH-MUX output
    ...
    {
        tp28xx_byte_write(addr, 0xF3, 0x00);
        tp28xx_byte_write(addr, 0xF5, 0x00);
        tp28xx_byte_write(addr, 0xFA, 0x00);
