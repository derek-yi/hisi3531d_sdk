...
static void usage(void)
{
    printf(
        ...
        "Usage: ./vou_chn_dump [VoLayer] [VoChn] [Frmcnt].\n"
        #else
        "Usage: vou_chn_dump [VoLayer] [VoChn] [Frmcnt].\n"
        #endif
        "1)VoLayer: \n"
        ...
        "*)Example:\n"
        #ifndef __HuaweiLite__
        "   e.g : ./vou_chn_dump 0 0 1 (dump one YUV)\n"
        #else
        "   e.g : vou_chn_dump 0 0 1 (dump one YUV)\n"
        #endif
        "*************************************************\n"
...
void VOU_Chn_HandleSig(HI_S32 signo)
{
    if (SIGINT == signo || SIGTERM == signo)
    {
        ...
        {
            HI_MPI_VO_ReleaseChnFrame(g_VoLayer, g_VoChn, &g_stFrame);
...
HI_S32 SAMPLE_MISC_VoDump(VO_LAYER VoLayer, VO_CHN VoChn, HI_U32 u32Cnt)
{
 ...
 //   printf("usage: ./vou_chn_dump [VoLayer] [vochn] [frmcnt]. sample: ./vou_chn_dump 0 0 5\n\n");
    ...
    sprintf(szYuvName, "./vo(%d,%d)_%s_%u_%ux%u.yuv",VoLayer,VoChn,
...
HI_S32 main(int argc, char *argv[])
{
    ...
    printf("\nNOTICE: This tool only can be used for TESTING !!!\n");
    printf("\tTo see more usage, please enter: ./vou_chn_dump -h\n\n");
