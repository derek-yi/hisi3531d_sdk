        ...
        HI_S32 s32Size, HI_S32 *pOffset)
{
    ...
    *pOffset = -1;
    ...
    /* The Circule Buffer must be twice as much as the data lenghth */
    ...
    else/* s32ReadPos is smaller than the s32WritePos */
    {
...
HI_S32 PCIV_Trans_InitSender(PCIV_TRANS_ATTR_S *pstAttr, HI_VOID **ppSender)
{
    ...
    pstSender->stRmtBuf.s32BaseAddr     = pstAttr->u32PhyAddr;
    ...
    u8VirtAddr = (HI_U8 *)HI_MPI_SYS_Mmap(u32PhyAddr, pstAttr->u32BufSize);
    if (NULL == u8VirtAddr)
    {
    ...
    pstSender->stLocBuf.u32PhyAddr  = u32PhyAddr;
    ...
    printf("%s ok, handle:0x%x, remote:%d, dma_buf:0x%x,len:%d; tmp_buf:(0x%x,%p), msgport:(%d,%d)\n",
        __FUNCTION__, (int)pstSender,pstSender->s32RmtChip, pstSender->stRmtBuf.s32BaseAddr, pstSender->stRmtBuf.s32Length,
        pstSender->stLocBuf.u32PhyAddr, pstSender->stLocBuf.pBaseAddr, pstSender->s32MsgPortWrite, pstSender->s32MsgPortRead);
...
HI_S32 PCIV_Trans_InitReceiver(PCIV_TRANS_ATTR_S *pstAttr, HI_VOID **ppReceiver)
{
    ...
    pVirAddr = (HI_VOID *)HI_MPI_SYS_Mmap(pstAttr->u32PhyAddr, pstAttr->u32BufSize);
    if (NULL == pVirAddr)
    {
        return HI_FAILURE;
    }
    ...
    *(HI_U32*)pVirAddr = 0x777777;
    ...
    printf("%s ok, remote:%d, addr:(0x%x,%p), buflen:%d, msgport:(%d,%d)\n",
        __FUNCTION__, pRev->s32RmtChip, pstAttr->u32PhyAddr, pRev->pu8BufBaseAddr,
...
HI_S32 PCIV_Trans_SendData(HI_VOID *pSender)
{
    ...
    stDmaBlk[0].u32SrcAddr = pstSender->stLocBuf.u32PhyAddr;
    stDmaBlk[0].u32DstAddr = pstSender->stRmtBuf.s32BaseAddr + s32WriteOff;
...
HI_S32 PCIV_Trans_GetData(HI_VOID *pReceiver, HI_U8 **pu8Addr, HI_U32 *pu32Len)
{
    ...
    HI_ASSERT((pStreamInfo->s32Start >= 0) && (pStreamInfo->s32End > pStreamInfo->s32Start)
