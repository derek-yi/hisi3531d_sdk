...
HI_VOID SAMPLE_IVE_TestMemory(HI_VOID)
{
	...
	HI_CHAR *pchSrcFile = "./data/input/testmem/test_mem_in_720x576.yuv";
	HI_CHAR *pchDstFile = "./data/output/testmem/test_mem_out_720x576.yuv";

    memset(&s_stTestMem,0,sizeof(s_stTestMem));
    SAMPLE_COMM_IVE_CheckIveMpiInit();
    s32Ret = SAMPLE_IVE_TestMemory_Init(&s_stTestMem,pchSrcFile,pchDstFile,u16Width,u16Height);
    if(HI_SUCCESS != s32Ret)
    {
    ...
    SAMPLE_IVE_TestMemoryProc(&s_stTestMem);
    
    SAMPLE_IVE_TestMemory_Uninit(&s_stTestMem);
    memset(&s_stTestMem,0,sizeof(s_stTestMem));

	SAMPLE_PRT("SAMPLE_IVE_TestMemory finished!\n");
