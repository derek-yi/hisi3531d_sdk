...
static SAMPLE_IVE_BG_MODEL_S s_stBgModel;
static SAMPLE_IVE_BG_MODEL_THREAD_PARAM_S s_stBgModelThreadParam;
...
{	
    ...
    prctl(PR_SET_NAME, "hi_BgM_Process", 0, 0, 0);
	...
	{				
		...
		for(i=0; i<u32VpssGrpCnt; i++)
		{
			...
			SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS!=s32Ret, FAIL_BG, "SAMPLE_IVE_BgModel_DetectObj failed with errno: %#x!\n", s32Ret);
...
HI_VOID SAMPLE_IVE_BgModel_ThreadParam(SAMPLE_IVE_BG_MODEL_THREAD_PARAM_S *pstThreadParam,
	...
	VENC_CHN VeH264Chn, VO_DEV VoDev, VO_LAYER VoLayer, VO_CHN VoChn, SAMPLE_IVE_BG_MODEL_S *pstBgModel)
{
	HI_S32 i;
...
HI_S32 SAMPLE_IVE_BgModel(HI_BOOL bEncode, HI_BOOL bVo)
{
    ...
    memset(&s_stBgModelThreadParam, 0, sizeof(s_stBgModelThreadParam));
    ...
    *****************************************/    	
    ...
    /******************************************
     step  3: Init BgModel
    ******************************************/
	s32Ret = SAMPLE_IVE_BgModel_Init(&s_stBgModel,astSize[1].u32Width,astSize[1].u32Height);
    ...
    ******************************************/
    SAMPLE_IVE_BgModel_ThreadParam(&s_stBgModelThreadParam,bEncode,bVo,u32VpssGrpCnt,VPSS_CHN_NUM,
    						  aVpssChn,VeH264Chn, VoDev, VoLayer,VoChn, &s_stBgModel);
    SAMPLE_IVE_BgModel_ThreadProc(&s_stBgModelThreadParam, &s_hIveThread);
   
    /******************************************
    step 11: exit process
    ******************************************/
	SAMPLE_IVE_BgModel_CmdCtrl(&s_stBgModelThreadParam, s_hIveThread);
...
END_BG_INIT:
	SAMPLE_IVE_BgModel_Uninit(&s_stBgModel);
...
HI_VOID SAMPLE_IVE_BgModel_HandleSig(HI_VOID)
{
	...
	s_stBgModelThreadParam.bFdStop = HI_TRUE;
	if (0 != s_hIveThread)
	{
	...
	if(s_stBgModelThreadParam.bEncode)
    {
    	SAMPLE_COMM_VENC_StopGetStream();
    }
    SAMPLE_COMM_VDEC_StopSendStream(s_u32VdecChnCnt,&s_stVdecSendParam,&s_VdecThread);
	...
	if(s_stBgModelThreadParam.bEncode)
	{       
    	SAMPLE_COMM_VENC_Stop(s_stBgModelThreadParam.VeH264Chn);
	}
	
	if(s_stBgModelThreadParam.bVo)
	{
    	SAMPLE_COMM_VO_StopChn(s_stBgModelThreadParam.VoLayer, s_enSampleVoMode);
    	SAMPLE_COMM_VO_StopLayer(s_stBgModelThreadParam.VoLayer);
    	SAMPLE_COMM_VO_HdmiStop();
    	SAMPLE_COMM_VO_StopDev(s_stBgModelThreadParam.VoDev);
	}
    ...
    SAMPLE_COMM_IVS_VpssStop(s_stBgModelThreadParam.u32VpssGrpCnt,VPSS_MAX_CHN_NUM);
    SAMPLE_COMM_VDEC_Stop(s_u32VdecChnCnt);
