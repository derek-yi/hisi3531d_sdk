...
static SAMPLE_IVE_GMM_S s_stGmm;
static SAMPLE_IVE_GMM_THREAD_PARAM_S s_stGmmThreadParam;
...
{	
    ...
    prctl(PR_SET_NAME, "hi_GMM_Process", 0, 0, 0);
...
HI_VOID SAMPLE_IVE_GMM_ThreadParam(SAMPLE_IVE_GMM_THREAD_PARAM_S *pstThreadParam,
	...
	VENC_CHN VeH264Chn, VO_DEV VoDev,VO_LAYER VoLayer, VO_CHN VoChn, SAMPLE_IVE_GMM_S *pstGmm)
{
	HI_S32 i;
...
HI_S32 SAMPLE_IVE_GMM(HI_BOOL bEncode, HI_BOOL bVo)
{
    ...
    memset(&s_stGmmThreadParam, 0, sizeof(s_stGmmThreadParam));
    ...
    *****************************************/    	
    ...
    /******************************************
     step  3: Init GMM
    ******************************************/
	s32Ret = SAMPLE_IVE_GMM_Init(&s_stGmm, astSize[1].u32Width,astSize[1].u32Height);
    ...
    ******************************************/
    SAMPLE_IVE_GMM_ThreadParam(&s_stGmmThreadParam,bEncode,bVo,u32VpssGrpCnt,VPSS_CHN_NUM,
    						  aVpssChn,VeH264Chn,VoDev, VoLayer,VoChn, &s_stGmm);
    SAMPLE_IVE_GMM_ThreadProc(&s_stGmmThreadParam, &s_hIveThread);
   
    /******************************************
    step 11: exit process
    ******************************************/
	SAMPLE_IVE_GMM_CmdCtrl(&s_stGmmThreadParam, s_hIveThread);
...
{
    HI_U32 i;
	s_stGmmThreadParam.bStop = HI_TRUE;
	if (0 != s_hIveThread)
	{
	...
	if(s_stGmmThreadParam.bEncode)
	{       
    	SAMPLE_COMM_VENC_Stop(s_stGmmThreadParam.VeH264Chn);
	}
	
	if(s_stGmmThreadParam.bVo)
	{
    	SAMPLE_COMM_VO_StopChn(s_stGmmThreadParam.VoLayer, s_enSampleVoMode); 
    	SAMPLE_COMM_VO_StopLayer(s_stGmmThreadParam.VoLayer);
    	SAMPLE_COMM_VO_HdmiStop();
    	SAMPLE_COMM_VO_StopDev(s_stGmmThreadParam.VoDev);
	}
    ...
    SAMPLE_COMM_IVS_VpssStop(s_stGmmThreadParam.u32VpssGrpCnt,VPSS_MAX_CHN_NUM);	
    SAMPLE_COMM_VDEC_Stop(s_u32VdecChnCnt);
	SAMPLE_IVE_GMM_Uninit(&s_stGmm);
    SAMPLE_COMM_SYS_Exit();
}
