...
HI_VOID SAMPLE_IVE_Canny(HI_CHAR chComplete)
{
	...
	HI_CHAR *pchSrcFileName = "./data/input/canny/canny_720x576.yuv";
	...
	sprintf(achDstFileName,"./data/output/canny/cannyout_complete_%c_720x576.yuv",chComplete);
    s32Ret = SAMPLE_IVE_Canny_Init(&s_stCannyInfo,pchSrcFileName,achDstFileName,u16Width,u16Height);
    if(HI_SUCCESS != s32Ret)
    {
    ...
    if('0' == chComplete)
    {
        SAMPLE_IVE_Part_Canny(&s_stCannyInfo);
    }
    else
    {
        SAMPLE_IVE_Complate_Canny(&s_stCannyInfo);
    }
   
    SAMPLE_IVE_Canny_Uninit(&s_stCannyInfo);
    memset(&s_stCannyInfo,0,sizeof(s_stCannyInfo));

	SAMPLE_PRT(" SAMPLE_IVE_Canny finished!\n");
