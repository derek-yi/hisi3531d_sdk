/******************************************************************************
  A simple program of Hisilicon Hi35xx osd implementation.
  the flow as follows:
    1) init mpp system.
...
HI_VOID SAMPLE_RGN_Usage(HI_VOID)
{
	...
	printf("\t 3)   VO: file->VDEC->VO(COVEREX+OVERLAYEX+LINE)     HDMI VGA 720P60\n");
...
HI_S32 SAMPLE_RGN_DetachFromChn(RGN_HANDLE Handle, HI_U32 u32Num,MPP_CHN_S stChn)
{
    HI_S32 i;
    HI_S32 s32Ret;
    for (i=Handle; i<(Handle + u32Num); i++)
    {
        s32Ret = HI_MPI_RGN_DetachFromChn(i, &stChn);
        if(s32Ret != HI_SUCCESS)
        {
            printf("HI_MPI_RGN_DetachFromChn fail! s32Ret: 0x%x.\n", s32Ret);
            return s32Ret;
        }
    }
    return HI_SUCCESS;
...
HI_S32 SAMPLE_RGN_CreateMosaicForVpss(RGN_HANDLE Handle, HI_U32 u32Num)
{
    ...
    for (i=Handle; i<(Handle + u32Num); i++)
    {
		...
		else if ((i - Handle) == 1)
		{
            ...
            stChnAttr.unChnAttr.stMosaicChn.stRect.s32X      = 640;
            stChnAttr.unChnAttr.stMosaicChn.stRect.s32Y      = 0;
		}
		else if ((i - Handle) == 2)
		{
            ...
            stChnAttr.unChnAttr.stMosaicChn.stRect.s32X      = 0;
            stChnAttr.unChnAttr.stMosaicChn.stRect.s32Y      = 360;
		}
		else
		{
			stChnAttr.unChnAttr.stMosaicChn.enBlkSize = MOSAIC_BLK_SIZE_32;
...
void* SAMPLE_RGN_VpssCoverDynamicDisplay(void* p)
{
    ...
    prctl(PR_SET_NAME, "hi_VpssCoverDyn", 0, 0, 0);
...
void* SAMPLE_RGN_VpssOSdDynamicDisplay(void* p)
{
    ...
    prctl(PR_SET_NAME, "hi_VpssOSdDyn", 0, 0, 0);
...
void* SAMPLE_RGN_VencOSdDynamicDisplay(void* p)
{
    ...
    prctl(PR_SET_NAME, "hi_VencOSdDyn", 0, 0, 0);
...
void* SAMPLE_RGN_VpssOverlayexDynamicDisplay(void* p)
{
    ...
    prctl(PR_SET_NAME, "hi_OverlayexDyn", 0, 0, 0);
...
void* SAMPLE_RGN_VpssCoverexDynamicDisplay(void* p)
{
    ...
    prctl(PR_SET_NAME, "hi_CoverexDyn", 0, 0, 0);
...
void* SAMPLE_RGN_VpssMosaicDisplay(void* p)
{
    ...
    prctl(PR_SET_NAME, "hi_MosaicDisplay", 0, 0, 0);
    ...
    while (HI_FALSE == bExit)
    {
        for (Handle=startHandle; Handle<(startHandle+u32RgnNum); Handle++)
        {
    		...
    		stChnAttr.bShow  = HI_TRUE;
            if(0==Handle-startHandle)
            {
                srand((int)time(NULL));
                stChnAttr.unChnAttr.stMosaicChn.stRect.s32X = 0+(((HI_U32)(576.0*rand()/(RAND_MAX+1.0)))/4)*4;
                stChnAttr.unChnAttr.stMosaicChn.stRect.s32Y = 0+(((HI_U32)(296.0*rand()/(RAND_MAX+1.0)))/4)*4;

                s32Ret = HI_MPI_RGN_SetDisplayAttr(Handle, &stChn, &stChnAttr);
            }
            else if(1==Handle-startHandle)
            {
                srand((int)time(NULL));
                stChnAttr.unChnAttr.stMosaicChn.stRect.s32X = 640+(((HI_U32)(576.0*rand()/(RAND_MAX+1.0)))/4)*4;
                stChnAttr.unChnAttr.stMosaicChn.stRect.s32Y = 0+(((HI_U32)(296.0*rand()/(RAND_MAX+1.0)))/4)*4;

                s32Ret = HI_MPI_RGN_SetDisplayAttr(Handle, &stChn, &stChnAttr);
            }
            else if(2==Handle-startHandle)
            {
                srand((int)time(NULL));
                stChnAttr.unChnAttr.stMosaicChn.stRect.s32X = 0+(((HI_U32)(576.0*rand()/(RAND_MAX+1.0)))/4)*4;
                stChnAttr.unChnAttr.stMosaicChn.stRect.s32Y = 360+(((HI_U32)(296.0*rand()/(RAND_MAX+1.0)))/4)*4;
...
void* SAMPLE_RGN_VdecSendStream(void* p)
{
    ...
    prctl(PR_SET_NAME, "hi_VdecSendStream", 0, 0, 0);
...
HI_VOID *SAMPLE_RGN_VpssOsdReverse(void *pData)
{
    ...
    prctl(PR_SET_NAME, "hi_OsdReverse", 0, 0, 0);
...
END_OC_VPSS9:
    ...
    stChn.s32DevId = 0;
    SAMPLE_RGN_DetachFromChn(OverlayHandle, u32RgnOverlayNum,stChn);
    SAMPLE_RGN_DetachFromChn(CoverHandle, u32RgnCoverNum,stChn);
...
HI_S32 SAMPLE_RGN_AddCoverOsdAndLineToVpss(HI_VOID)
{
    ...
    *************************************************/
    ...
    stChn.s32ChnId = VPSS_CHN0;
    stChn.s32DevId = 0;
    SAMPLE_RGN_DetachFromChn(OverlayexHandle, u32OverlayexRgnNum,stChn);
    SAMPLE_RGN_DetachFromChn(CoverexHandle, u32CoverexRgnNum,stChn);
    SAMPLE_RGN_DetachFromChn(LineHandle, u32LineRgnNum,stChn);
...
HI_S32 SAMPLE_RGN_AddMosaicToVpss(HI_VOID)
{
    ...
    *************************************************/
    ...
    stChn.s32ChnId = 0;
    stChn.s32DevId = 0;
    SAMPLE_RGN_DetachFromChn(MosaicHandle, u32MosaicNum,stChn);
