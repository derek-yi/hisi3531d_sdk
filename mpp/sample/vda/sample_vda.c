...
HI_S32 SAMPLE_VDA_SYS_Init(HI_VOID)
{
    ...
    struVbConf.astCommPool[1].u32BlkCnt  = 15;
...
HI_S32 SAMPLE_VDA_StartVpss(HI_S32 s32VpssGrpNum, HI_U32 u32VpssChn)
{
    ...
    for (i = 0; i < s32VpssGrpNum; i++)
    {
        ...
        stVpssFrameRate.s32DstFrmRate = 25;
        stVpssFrameRate.s32SrcFrmRate = 100;
        s32Ret = HI_MPI_VPSS_SetGrpFrameRate(i, &stVpssFrameRate);
        if (HI_SUCCESS != s32Ret)
        {
            printf("SetGrpFrameRate grp%d fail, s32Ret: 0x%x.\n", i, s32Ret);
            return s32Ret;
        }
...
HI_S32 SAMPLE_VDA_StartVdec(VDEC_CHN VdecChn)
{
    ...
    s32Ret = HI_MPI_VDEC_SetDisplayMode(VdecChn, VIDEO_DISPLAY_MODE_PREVIEW);
    if (HI_SUCCESS != s32Ret)
    {
        printf("HI_MPI_VDEC_SetDisplayMode failed! s32Ret:0x%x.\n", s32Ret);
        return s32Ret;
    } 
...

{
    ...
    *************************************************/
    SAMPLE_COMM_VDA_MdStop(VdaChn_Md, VdecChn);
